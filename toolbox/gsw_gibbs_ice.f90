...
real (r14) :: dzi, g0, g0p, g0pp, sqrec_pt
complex (r14) :: r2, r2p, r2pp, g, sqtau_t1, sqtau_t2, tau, tau_t1, tau_t2

real (r14), parameter :: s0 = -3.32733756492168d3
...
if (nt.eq.0 .and. np.eq.0) then
    ...
    g = r1*(tau*log((1.d0 + tau_t1)/(1.d0 - tau_t1)) &
        + t1*(log(1.d0 - sqtau_t1) - sqtau_t1)) &
	+ r2*(tau*log((1.d0 + tau_t2)/(1.d0 - tau_t2)) &
	+ t2*(log(1.d0 - sqtau_t2) - sqtau_t2))
...
elseif (nt.eq.1 .and. np.eq.0) then
    ...
    g = r1*(log((1d0 + tau_t1)/(1d0 - tau_t1)) - 2d0*tau_t1) &
        + r2*(log((1d0 + tau_t2)/(1d0 - tau_t2)) - 2d0*tau_t2)
...
elseif (nt.eq.0 .and. np.eq.1) then
    ...
    g0p = rec_pt*(g01 + dzi*(2.d0*g02 + dzi*(3.d0*g03 + 4.d0*g04*dzi)))
    
    r2p = rec_pt*(r21 + 2.d0*r22*dzi)
        
    g = r2p*(tau*log((1.d0 + tau_t2)/(1.d0 - tau_t2)) &
        + t2*(log(1.d0 - sqtau_t2) - sqtau_t2))
...
elseif (nt.eq.1 .and. np.eq.1) then
    ...
    g = r2p*(log((1.d0 + tau_t2)/(1.d0 - tau_t2)) - 2.d0*tau_t2)
...
elseif (nt.eq.2 .and. np.eq.0) then
    ...
    g = r1*(1.d0/(t1 - tau) + 1.d0/(t1 + tau) - 2.d0/t1) &
        + r2*(1.d0/(t2 - tau) + 1.d0/(t2 + tau) - 2.d0/t2)
...
elseif (nt.eq.0 .and. np.eq.2) then
    ...
    g0pp = sqrec_pt*(2.d0*g02 + dzi*(6.d0*g03 + 12.d0*g04*dzi))
    
    r2pp = 2.d0*r22*sqrec_pt
       
    g = r2pp*(tau*log((1.d0 + tau_t2)/(1.d0 - tau_t2)) &
        + t2*(log(1.d0 - sqtau_t2) - sqtau_t2))
