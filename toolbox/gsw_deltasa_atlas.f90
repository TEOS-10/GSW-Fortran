...
real (r14) :: dlong, dlat
real (r14) :: p0_original, lon0_in, sa_upper, sa_lower 
...
ndepth_max = -1
do k = 1, 4
   if (ndepth_ref(indy0+delj(k),indx0+deli(k)).gt.0d0) &
...
sa_upper = (1d0-s1)*(dsar(1) + r1*(dsar(2)-dsar(1))) + s1*(dsar(4) + &
		r1*(dsar(3)-dsar(4)))
